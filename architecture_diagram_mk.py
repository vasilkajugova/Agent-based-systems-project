"""
Macedonian-labeled architecture diagram (Figure 1 in the thesis).
Rebuilds results/figures/architecture_diagram.png from scratch with
matplotlib, using the same box/arrow layout as architecture_diagram_en.py
so the two language versions render identically apart from the text.
"""
...
OUT = Path(__file__).parent / "results" / "figures" / "architecture_diagram.png"
...
# Title
ax.text(50, 66, "Архитектура: од highway-env до тренираните агенти",
...
ax.text(50, top_y + top_h * 0.27, "kinematic bicycle модел   +   IDM/MOBIL (human-driven возила)",
...
bullets = [
    "flatten Kinematics опсервација по агент   (obs_dim = 42)",
    "per-agent награда преку env.unwrapped._agent_reward(a, v)",
    "+ courtesy penalty (казна за агресивно возење блиску до human возила)",
    "active-masking:  0 награда за агент кој веќе завршил (поправка на bug-от)",
]
by0 = mid_y + mid_h - 6.6
...
ax.text(
    50, 0.9,
    "a = (a_1, ..., a_n)   →   враќање кон env.step(a)   "
    "(decentralized execution: секој агент одлучува само од o_i)",
