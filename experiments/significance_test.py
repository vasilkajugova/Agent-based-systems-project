...
известувам средина ± 95% доверителен интервал низ seed-овите. Преклопување на
95% CI е добар ВИЗУЕЛЕН индикатор, но не е формален статистички тест - затоа
тука го пресметувам Welch's t-test (не претпоставува еднакви варијанси помеѓу
групите, за разлика од обичен Student's t-test - соодветно тука бидејќи бројот
на епизоди/варијансата на VDN и IQL не мора да е ист) директно врз 5-те per-seed
...
ВАЖНО (додадено при критичка ревизија на проектот, по последниот retrain):
претходна верзија на овој коментар тврдеше "IQL и VDN имаат статистички
неразличлива просечна награда, но значително различна стапка на судири/успех"
- тоа НЕ се совпаѓа со тековниот results/significance_test.json. Со
тековните бројки (n=5 seed-ови), IQL vs VDN НЕ е статистички значајно ниту
на mean_reward (p≈0.35), ниту на collision_rate (p≈0.075, гранично), ниту на
arrival_rate (p≈0.27) - значаен е единствено avg_steps (p≈0.006, VDN
епизодите траат подолго, насоката сама по себе не значи "подобро"). И IQL и
VDN значајно го бијат heuristic baseline-от на сите метрики освен
VDN-vs-heuristic arrival_rate (p≈0.087, единствениот незначаен резултат
таму - IQL-vs-heuristic arrival_rate Е значаен, p≈0.0005). Ако извештајот/трудот некаде ја повторува
старата формулација, ТРЕБА преформулирана да одговара на овие бројки - не ги
хардкодирам тука намерно (веќе еднаш застарија по промена во кодот), секогаш
цитирај директно од results/significance_test.json пред секое тврдење во
трудот. Види и experiments/confound_check_study.py - контролна студија која
дополнително проверува дали IQL vs VDN разликата (или нејзиниот недостаток)
доаѓа реално од CTDE, или делумно од архитектурен конфаунд (dueling/double-DQN
асиметрија во главната споредба).
