...
from agents.networks import QNetwork, DuelingQNetwork, ReplayBuffer
...
class DQNAgent:
    def __init__(
        ...
        self.model = net_cls(obs_dim, num_actions, hidden).to(self.device)
        self.target_model = net_cls(obs_dim, num_actions, hidden).to(self.device)
