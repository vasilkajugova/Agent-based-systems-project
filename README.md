...
# Мултиагентно учење со поттикнување за координација на автономни возила на раскрсница
...
голема варијанса по seed, особено кај IQL). **Забелешка за
репродуцибилност**: овие конкретни проценти се од порано (еднократно)
пуштање - тековниот `experiments/robustness_study.py --obs_modes` сè
уште прифаќа само `kinematics`/`pixels`/`fusion` (не и `fusion_fixed`
модели), па `results/robustness_study.json` во репозиториумот моментално
не содржи `fusion_fixed` редови за директна проверка; треба или скриптата
да се прошири со поддршка за `fusion_fixed`, или бројките експлицитно да
се означат како историски/нерепродуцирани од тековниот код.

Заклучок: поправката РЕАЛНО ја принудува мрежата да ја користи сликата
кај повеќето (но не сите) агент×seed комбинации - img_sens расте за
~2-3 реда на големина во просек, но со забележлива нестабилност (некои
агенти сепак остануваат близу collapse дури и со поправката). Тоа доаѓа
